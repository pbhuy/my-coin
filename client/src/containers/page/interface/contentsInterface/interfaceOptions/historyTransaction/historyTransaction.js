...
export const HistoryTransaction = ({ pendingTx, stateMine }) => {
  ...
  useEffect(() => {
    const fetchDate = async () => {
      const result = await axios.get(`/transactions/${myWallet.publicKey.substring(2, myWallet.publicKey.length)}`);
      ...
      <div className='history-transaction__body'>
        ...
        <div className={`cover ${pagination.length > 1 ? '' : 'cover--hidden'}`}>
          <div className='pagination'>
            {pagination.map((item) => {
              return (
                <div data-id={item.id} className={`pagination__item ${item.active}`} onClick={handlePagination}>
                  {item.id}
                </div>
              );
            })}
